  ...
  <body>
    ...
    <xsl:for-each select="$body-cells">
        ...
        <xsl:if test="steptitle">
            ...
            <xsl:if test="not(steptitle[@style = 'document'])">
                <xsl:value-of select="'ENDHTML ## '"/>
            </xsl:if>
            <xsl:apply-templates select="steptitle"/>
            <xsl:if test="not(steptitle[@style = 'document'])">
                <xsl:value-of select="'STARTHTML '"/>
            </xsl:if>
        </xsl:if>
...
<xsl:template match="mcode-xmlized">
  <pre class="codeinput" style="white-space: pre-wrap; word-wrap: break-word;"><xsl:apply-templates/><xsl:text><!-- g162495 -->
</xsl:text></pre>
</xsl:template>
